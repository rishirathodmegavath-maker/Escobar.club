...
export function DiscoverCampaignCard({ campaign }: { campaign: Campaign }) {
  return (
    <Link
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-col items-end gap-1.5">
          {campaign.adminDisplayStatus ? (
            ...
            <>
              {campaign.hot && (
                <span className="inline-flex items-center gap-1 rounded-full bg-alert-soft px-2.5 py-1 font-mono text-xs font-semibold uppercase tracking-wide text-alert-deep">
                  <BoltIcon className="h-3 w-3" strokeWidth={2.2} />
                  Priority
                </span>
              )}
